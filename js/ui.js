...
function toggleCard(key, el) {
  ...
  body.style.display = isOpen ? "none" : ""; if (preview) preview.style.display = isOpen ? "" : "none"; if (!isOpen) { const bd = document.createElement("div"); bd.className = "modal-backdrop"; bd.id = "modalBackdrop"; bd.style.cssText = "position:fixed;inset:0;z-index:199;background:rgba(0,0,0,0.001);"; bd.addEventListener("click", (e) => { e.stopPropagation(); toggleCard(key, el); }); document.body.appendChild(bd); card.classList.add("card-expanded"); document.body.classList.add("card-modal-open");
