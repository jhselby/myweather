            ...
            <div onclick="document.getElementById('${groupId}').style.display = document.getElementById('${groupId}').style.display === 'none' ? 'block' : 'none'; this.querySelector('.bird-chev').textContent = document.getElementById('${groupId}').style.display === 'none' ? '▾' : '▴';"
              ...
              <div style="min-width:0;flex:1;">
                <div style="font-weight:700;font-size:0.9rem;color:${textHead};overflow:hidden;text-overflow:ellipsis;white-space:nowrap;">
                  ${loc.loc_id && !loc.loc_private ? `<a href="https://ebird.org/hotspots?hs=${loc.loc_id}" target="_blank" rel="noopener" onclick="event.stopPropagation(); event.preventDefault(); window.__externalLinkOpen = true; window.open('https://ebird.org/hotspots?hs=${loc.loc_id}', '_blank');" style="color:${textHead};text-decoration:none;border-bottom:1px dotted ${textFaint};">${escapeHtml(loc.name)}</a>` : loc.loc_private && loc.lat && loc.lng ? `<a href="maps:?q=${encodeURIComponent(loc.name)}&ll=${loc.lat},${loc.lng}&z=15" target="_blank" rel="noopener" onclick="event.stopPropagation(); event.preventDefault(); window.__externalLinkOpen = true; window.location.href = 'maps:?q=${encodeURIComponent(loc.name)}&ll=${loc.lat},${loc.lng}&z=15';" style="color:${textHead};text-decoration:none;border-bottom:1px dotted ${textFaint};">${escapeHtml(loc.name)}</a>` : escapeHtml(loc.name)}${locNotables > 0 ? ` <span style="background:${notableBg};color:${notableFg};padding:1px 6px;border-radius:999px;font-size:0.7rem;font-weight:700;margin-left:4px;">${locNotables}★</span>` : ""}
                </div>
