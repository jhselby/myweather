...
**Piece 3 — Fitter.** Once a day, at 3:07 AM Eastern, the collector reads the full pair log, groups by (field, lead-hour), and computes the mean error per bin. The result is `decay_corrections.json` — a small lookup table of 6 fields × 48 lead-hour bins. Same pass also prunes the pair log to the 30-day window and rewrites it as a single non-composed blob.
