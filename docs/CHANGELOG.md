...
- **Why:** Pipeline Lift tile description says "Prod vs L1 selector's pick... on top of raw" — that description is honest only when `L1_selected = raw`. The pre-v0.6.555 walk made both sides asymmetric (HRRR side: L1_selected = raw HRRR, every layer above counts as correction; NBM side: L1_selected = L3_NBM, only L4+ counted as correction). Now symmetric: L1_selected IS the raw of the picked source; every Wyman Cove layer above raw (L2_NBM/L3_NBM/L4_NBM/L5_NBM/L6_NBM/chp_nbm/wdp_nbm on NBM; L2/L3/L4/L5/L6/chp/dpbp/wsbp/wdp/clp on HRRR) counts as our correction. Also fixes the semantic mis-labeling that called L3_NBM "NBM's own bias-corrected output" in the docstring — factually wrong; it's ours.
- **Expected effect:** Pipeline Lift 7d numbers climb on fields where L3_NBM stamps (currently `L3_NBM_FIELDS = ("wg", "ch", "cc", "sr")` after the 09-05 h kill). Total Lift unchanged. Selector Skill (`chooser_vs_prod_pct`) unchanged. Hit Rate + Value Captured unchanged. Comparisons to pre-shift Pipeline Lift values are not directly meaningful — new baseline starts today.
- **Full scoring audit shipped alongside** (per_field_scoring.py + scoreboard_v2.py + mae_over_time.py + h_persistence_skill.py + pp_brier_reliability.py + nbm_l2_delta_audit.py + gate_firing_rollup.py). Four stale docstrings in per_field_scoring fixed (header, `_selected_l1_error` inline, conventions block, warmup_note, HRRR-side counterfactual "deferred" comment now moot). Flagged for discussion but not shipped: (A) scoreboard_v2 baseline `argmin(HRRR, NBM)` contradicts per_field_scoring's user-default baseline `NBM raw`; (B) scoreboard_v2 has no current-config counterfactual; (C) `chooser_vs_prod_pct` is pooled MAE ratio while the tile description implies per-row rate (Hit Rate is the per-row rate); (D) 7d window has no fossil-row gate.
- **No collector deploy.** Analysis-only. Publisher CF redeployed earlier this session (revision `myweather-publisher-00018-vam` at 10:58 UTC) so the fix reaches production on next hourly tick.
...
- **L3_NBM scope references updated** in both the "NBM parallel cascade" summary and the "NBM-side direct grib ingester" ops line: <code>l3_nbm</code> for wg/h/ch/cc/sr (sr re-added 09-04 v0.6.549 on walkforward +12.8% aggregate, all four lead bands positive).
- **09-04 calendar item closed** — NBM POP first 7d of data has landed; digest's <code>pp_brier_reliability</code> reads CLEAN weighted_bias -1.44pp so no Stage 0 candidate on HRRR pp itself. Selector-eval script (HRRR pp vs NBM pp Brier-scored per lead-band) not built yet, scoped as follow-up. Companion <code>chp_nbm</code> 14d re-evaluation: sentry marginal help +23.8% → +31.5% (helping more in fresh) = healthy.
- **Stale "cc.l4_nbm HOT flag today" note removed** from the operational-state paragraph. Under the new marginal-help sentry cc.l4_nbm reads CLEAN; the follow-up track is the 09-09 DROP decision now backed by <code>[[l4-nbm-cc-drop-prep]]</code>.

</details>

<details>
<summary><strong>v0.6.549 • September 4, 2026 (L3_NBM adds sr — hour-of-day diurnal correction on NBM solar radiation, +12.8% aggregate lift)</strong></summary>

- **`weather_collector/processors/l3_nbm.py`** — `L3_NBM_FIELDS` gains `"sr"`. Walkforward validator has been proposing `l3_nbm: ADD sr` on n=2,367 paired rows with +12.8% aggregate lift and all four lead bands positive (0-5h +18.6%, 6-11h +19.4%, 12-23h +12.6%, 24-47h +8.2%). No hurting cells surfaced in skip-proposals for sr. This gives sr its first NBM-side hour-of-day correction — previously sr's NBM cascade dead-ended at raw + L2_nbm (L5_nbm sr was killed v0.6.471 08-25). Curated coefficients already emit by `analysis/l3_nbm_fit.py` (per-lead-hour, 48 bins, n_samples 574-626 for leads 0-33h, thinning to 84-368 at leads 34-47h). Corrections are large diurnal-shaped (subtract 37-52 W/m² for most short-mid lead bins, near-zero at long lead where NBM raw is already reasonable).
- **Selector interaction:** L1 selector fit already saw all four sr bands NBM-winning (per 09-03 v0.6.546 recency override — all four sr bands overridden HRRR→NBM on 7d evidence). Adding L3 correction on top will improve NBM's prod-side MAE further, likely making the NBM pick even more dominant for sr going forward. Expected: NBM sr lift widens on next selector refit; hit rate on the four flipped cells solidifies.
- **Collector deploy required** (runtime processor change).

</details>

<details>
<summary><strong>v0.6.548 • September 4, 2026 (NBM regression sentry — marginal layer effect metric filters raw-weather false positives)</strong></summary>

- **`analysis/nbm_regression_sentry.py` — verdict now uses marginal layer effect, not absolute cascade MAE.** Previously the sentry fired HOT/WATCH on `error_l{n}_nbm` MAE fresh 3d vs sustained 7d, which conflates layer regressions with underlying raw-weather difficulty shifts. Today's digest surfaced two false-positive HOTs (wd.l3_nbm ΔMAE +41%, wg.l3_nbm ΔMAE +58%) that were both driven by raw NBM MAE inflating in the fresh window (wd raw +34%, wg raw +72%). On the wg case the layer was actually helping MORE in the fresh window (help +16.4% vs +9.1%). New `LAYER_INPUT` map identifies each layer's cascade input (l3_nbm→l2_nbm, l4_nbm→l3_nbm, chp_nbm→l4_nbm, wdp_nbm→l3_nbm, etc.). For paired rows carrying both layer and input errors we compute `layer_help_pct = (input_MAE − layer_MAE) / input_MAE` per window and set the verdict on `Δhelp_pp = help_sustained − help_fresh`. HOT thresholds unchanged in name (≥15pp / ≥8pp) but now measure marginal degradation. HOT also fires if the layer flipped from net-helping to net-hurting even below the pp bar. Absolute ΔMAE% still reported for context; if a layer lacks input-pair data the verdict falls back to the old absolute rule with a `[absolute fallback]` note in exec-summary.
- **`analysis/runlog/build_executive_summary.py`** updated to render the new marginal fields: `⚠ h.l3_nbm: WATCH — layer help +20.5% → +10.4% (Δ +10.1pp; n_sust 6,374, n_fresh 2,739; vs l2_nbm)`. Today's re-run cleared both HOT flags (wd/wg both CLEAN under marginal metric) and surfaced one real WATCH on h.l3_nbm that was buried by the old absolute metric (h raw got easier, absolute ΔMAE −24.6% masked that the layer's own contribution halved from +20.5% to +10.4% of its input).
- **Zero runtime impact.** Analysis-only change; no collector deploy.

</details>

<details>
<summary><strong>v0.6.547 • September 3, 2026 (session-end debug page + memory sweep — v0.6.546 selector recency-override narrative)</strong></summary>

- **`corrections_debug.html` calendar refresh.** 09-03 entry updated in place: "1 ship, 0 collector deploys. Triage-focused Thu." → "2 ships + 1 collector deploy. Triage-focused Thu turned real afternoon ship." Appended v0.6.546 SHIP (afternoon) narrative — recency-override design, motivation (Selector Skill median -4.4%, hit rate 54.1%, value captured -28.4% median), 10 HRRR→NBM flips (sr all 4 bands, h 6-11 and 12-23, dp 0-5, t 24-47, wd 0-5) with ws 24-47 stability-floor behavior, ship gate +48.4% on n=72,475, deploy timing 00:18 UTC rev 00549-qug and 00:27 UTC first-run cold-start verification. Memory line updated to reference new watch.
- **Memory:** [[project_09_03_session]] updated with v0.6.546 as second ship (frontmatter description + Ships section + Lessons section covering the "capitulated to wait-for-gates all week when cross-cutting selector-level ship was actionable independently" pattern). New [[project_selector_recency_override_watch]] with three checkpoints: Fri 09-04 morning debug-page recovery (Selector Skill / Value Captured / Hit Rate should move toward flat-or-positive), 7-day re-check for override churn (if ≥3 cells churn, tighten MIN_LIFT_RECENT_PCT from 5% to 7-8%), 2-month quiet-evaporation as pre-v0.6.540 data ages out of the 30d fit pool (override_count should drop from 10 to 0-2 by mid-November; persistence past 11-04 means recency window needs to widen or a different bias is being papered over). MEMORY.md READ FIRST rewritten for the two-ship day.
