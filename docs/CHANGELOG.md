...
- **Briefing reliability pass.** Three changes after this morning's wave of Gemini 503/429 failures and a Groq fallback that hallucinated "Cloudy Now" when the sky was clear:
  ...
  - **Single 5-second retry on Gemini 503/429** in `briefing_ai.py`. Most transient capacity errors clear in seconds; previously the first 503 fell straight through to the Groq fallback. Now we try once more before falling back.
  - **Groq temperature lowered from 0.9 to 0.5** in `briefing_ai.py`. Llama-3.3 at 0.9 wandered into headlines that contradicted the structured weather data (the "Cloudy Now" / sky=Clear screenshot). 0.5 keeps natural English but cuts hallucination room. Gemini stays at 0.9 — its guardrails handle high temperature fine.
- **Collector memory bump 1024 → 1536 MB.** OOM crashes were happening every ~70 minutes since 06-07 (15h after v0.6.42 deployed). The combination of v0.6.42's longer fitter snapshot read + climbing failed-sensor retry buffers pushed runs over the 976 MiB ceiling. Bumped to 1536 MB; three consecutive clean runs since deploy at 13:07 UTC.
