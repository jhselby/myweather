...
## v0.6.95 • June 15, 2026
- **L5 evaluation script + the finding it immediately surfaced.** New `analysis/l5_solar_analysis.py` joins solar pairs from the held-out window against the L5 lookup, computes per-regime MAE before/after correction, in both "realistic" mode (regime from model forecast — what we'd actually do in production) and "ceiling" mode (regime from observation — theoretical best). Verdict: SHIP if realistic overall MAE drops ≥5% AND ≥5 of 8 regimes improve by ≥3%. First run on 7-day local data: **HOLD** — only sw_flow shows the predicted improvement (−7.5%), frontal regime actually gets *worse* (+13.3%), overall drop only 0.7%. Cause: the hardcoded biases came from state_stratified which averages across all hours including night; daytime-only bias is likely different, and small-sample regimes (frontal n=1769) too noisy to trust. **L5 needs design refinement before 06-22 ship/hold call — same shape as the R4 fetcher bug discovery this morning, but caught now in low-pressure prep time instead of at the decision point.** Possible iterations: recompute biases from daytime-only obs, add hour-of-day stratification within each regime, investigate over-correction in frontal regime.

## v0.6.94 • June 15, 2026
- **L5 solar regime correction — sketched, gated OFF (06-22 decision).** R2 state-stratified analysis ranks regime_synoptic for solar as the #1 opportunity across all (field, dimension) combinations: solar bias swings from +89 W/m² in frontal regime (model overpredicts — actual cloud thicker than model) to −32 W/m² in sw_flow (underpredicts — model clouds dissipate faster than reality). 121 W/m² systematic offset that L3/L4 can't see because they don't condition on regime. New `solar_correction.py` indexes a lookup table by `derived.state.regime_synoptic`, returns the negated bias as the candidate correction (suppressed below 50 W/m² to avoid "correcting" night). Stamps `weather_data["solar_correction"]` per tick; does NOT modify `direct_radiation` until ENABLED is flipped after the 06-22 evaluation. Same shape as the cove correction we shipped earlier — gated off, observable, validated later.
...
## v0.6.86 • June 15, 2026
- **Manual pair-log dedup — 4.6 GB → 819 MB instantly.** Instead of waiting 30 days for the v0.6.77 dedup to age in naturally, downloaded the current 4.8 GB log via curl, ran a one-shot Python dedup keeping the first row per (run_time, lead_h, field, obs_hour), uploaded the deduped 819 MB result as the production file. Backup of the original retained at `gs://myweather-data/forecast_error_log_pre_dedup_backup.jsonl` for 14 days. The 14:07 UTC Joiner appended pairs onto the old file during my upload window; those pairs were lost when my upload completed. Rewound `forecast_error_state.json` to allow the next Joiner run to reprocess the 10:07 EDT obs. Net win: backtest sweeps drop from ~7 min to ~5 sec immediately, walk-forward confidence intervals become honest immediately, GCS storage cost drops 6× from this moment forward. Notable side effect: MAE numbers across the board shifted slightly (most fields ~20-30% higher under honest single-sample-per-hour vs pre-dedup 6× averaging) — this is expected behavior of honest stats vs inflated, not a regression. One concrete signal change: walkforward_15jun verdict (drop cm from L3) now visibly wins by 2.4% under honest stats vs the 1.5% loss it appeared to be under inflated data.
