...
## v0.6.44 • June 8, 2026
- **L2 lead-decay + L3/L4 pause.** Audit of held-out per-layer MAE (time_series_diagnostic.json) showed L3 (decay) and L4 (diurnal) actively making temperature, humidity, dew point, solar, low cloud, pressure and precip amount *worse* than raw HRRR/GFS, while wind speed/gust and high/mid cloud were the only fields they helped. Root cause traced to L2: the Kalman-scaled station bias was applied flat across all 48 lead hours, but the bias's signal-to-noise degrades fast as the forecast moves into different atmospheric state. L3 then fits the residual of this over-applied bias, and ends up learning ~zero in the mean while individual days are still way off (regression-to-mean trap). New `analysis/l2_lead_decay_fit.py` fits a single τ per field on 73,510 train pairs: `bias_applied(lead) = bias × exp(-lead/τ)`. Held-out wins vs current flat L2: t +5.1% (τ=4h, the bias collapses to zero past lead ~12h), h +3.8% (τ=240h), pr +4.2% (τ=12h), dp +3.3% (inherited). Wind speed/gust prefer τ=∞ (current flat behavior remains correct). Solar/clouds/precip were already L2-no-ops in this window so τ is irrelevant. Productionized in `corrected_hourly.py` with inline default τ values + optional GCS override via `l2_decay.json`. L3 and L4 paused entirely via `APPLY_LAYER_3=False`/`APPLY_LAYER_4=False` switches in `decay_apply.py` for ~14 days so the recency-weighted fitter rebuilds against the new (correct-L2) residuals before re-enable; per-layer MAE snapshots still publish so the audit can be re-run cleanly. `decay_meta` gains `layer_3_paused` / `layer_4_paused` flags; `l2_decay_meta` carries per-field τ. Expect the next audit to show temp/humidity/pressure moving from net-negative to net-positive at the L2 layer alone.
