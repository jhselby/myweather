"""Stage 0 — temperature run-level bias carryover.

Motivated by smoke test in `analysis/h_smoke_C_deepdive.py` (08-10): after
de-meaning by (hour × lead-bucket), `t` shows 58% same-sign agreement between
each run's short-lead (0-3h) and long-lead (24-48h) residual means, with 23%
...
OUT_TXT = os.path.join(SCRIPT_DIR, "output", "h_run_bias_carryover_t_stage0.txt")
OUT_JSON = os.path.join(SCRIPT_DIR, "output", "h_run_bias_carryover_t_stage0.json")


def main():
    res = compute(field="t")
    return emit_and_write(res, OUT_TXT, OUT_JSON,
                          hypothesis_slug="run-bias carryover  (t)")
