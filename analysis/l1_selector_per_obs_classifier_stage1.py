"""Stage 1 v2 — per-obs continuous classifier for the L1 selector.

Motivation (per [[feedback_stage0_shadow_lift_gate_hrrr_dominant]]): on
HRRR-dominant fields (sr, t, ws) the Stage 0 quartile-bin gate is
structurally blind — HRRR wins cell-avg in every quartile of every cell,
so shadow_lift ≡ 0. But the per-obs oracle gap is real (10-30% MAE on t)
and H_win% varies 50-90% across xr_q / ims quartiles. A continuous
per-obs classifier can extract signal a bin-picker can't.

Rule shape: per (regime, band), fit L2-regularized logistic regression
on the train half:
    P(NBM wins | features) = sigmoid(β0 + β·x)
Features (all standardized on train):
    x = [ims, xr_spread, lead_h,
         sin(2π hour/24), cos(2π hour/24),
         cc_disagreement_fc_obs, cloud_inter_source_sigma,
         pressure_trend_hpa_3h]
Route the row to NBM iff P > θ, else HRRR. Sweep θ on train; take θ*
that minimizes train MAE. Apply θ* on test → test MAE.
...
MIN_N_TEST = 150
MIN_LIFT_PCT = 3.0
L2_LAMBDA = 3.0     # ridge on standardized features (bumped 09-20: 0.5 overfit)
IRLS_MAX_ITER = 50
IRLS_TOL = 1e-6
...
print("=" * 132)
print(f"l1_selector_per_obs_classifier_stage1 — logistic on 8 features, field={FIELD}")
print("=" * 132)
...
out_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "output",
                       f"l1_selector_per_obs_classifier_stage1_{FIELD}.json")
