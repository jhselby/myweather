"""
h (humidity) residual-persistence Stage 1 preview — tune window + verify robustness.

Follow-on to h_daily_residual_persistence Stage 0 finding (07-18 digest):
rolling 2-day mean L2-residual at same clock hour reduces h MAE -8.16% /
RMSE -8.40% held-out. Mirrors h_wg_residual_persistence_stage1.py exactly
(same template, different field). Grid-searches window size, checks per-
regime robustness, applies to Production (post-L3/L4) as well as L2 alone,
and halves-checks stability on training.
...
Run:
    python3 analysis/h_h_residual_persistence_stage1.py

Output:
    analysis/output/h_h_residual_persistence_stage1.txt
    analysis/output/h_h_residual_persistence_stage1.json
"""
import json
import os
import sys
...
OUT_TXT = os.path.join(SCRIPT_DIR, "output", "h_h_residual_persistence_stage1.txt")
OUT_JSON = os.path.join(SCRIPT_DIR, "output", "h_h_residual_persistence_stage1.json")
...
def load_rows():
    ...
    with open(cached_path(PAIR_LOG_URL), "rb") as fh:
        for raw in fh:
            ...
            # v0.6.520: reconstruct Production from applied_layer; top-level
            # `forecast` is L2 by design. Ports the v0.6.400a fix already in
            # the dp Stage 1 script — needed for honest Production-baseline
            # comparison.
