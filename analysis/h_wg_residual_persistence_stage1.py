"""
wg residual-persistence Stage 1 preview — tune window + verify robustness.

Follow-on to v0.6.342 Stage 0 finding: rolling 2-day mean L2-residual at
same clock hour reduces wg MAE −6.13% / RMSE −7.41% held-out. This script
tunes the window size, checks per-regime robustness, applies to Production
(post-L3) instead of L2 alone, and runs a halves-check on training.
...
Run:
    python3 analysis/h_wg_residual_persistence_stage1.py

Output:
    analysis/output/h_wg_residual_persistence_stage1.txt
    analysis/output/h_wg_residual_persistence_stage1.json
"""
import json
import os
import sys
...
OUT_TXT = os.path.join(SCRIPT_DIR, "output", "h_wg_residual_persistence_stage1.txt")
OUT_JSON = os.path.join(SCRIPT_DIR, "output", "h_wg_residual_persistence_stage1.json")
...
def main():
    rows = load_rows()
    if not rows:
        print("No wg rows in pair log; aborting.", file=sys.stderr)
        return 1
    ...
    emit("=" * 100)
    emit("wg RESIDUAL-PERSISTENCE — STAGE 1 PREVIEW")
    emit("=" * 100)
    emit(f"Total wg rows: {len(rows):,}  (train {len(train_rows):,} / test {len(test_rows):,})")
