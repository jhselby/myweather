"""Shared Stage 2 harness for residual-persistence hypotheses.

Extracted 2026-08-30 v0.6.524 after refactoring Stage 1 (v0.6.522) exposed
the "three cloned 400-line siblings" pattern here too — dp and wg Stage 2
were bit-identical modulo FIELD + units_label + output paths. Writing
h Stage 2 as a fourth clone would compound the same drift risk that just
paid off to fix.
...
Stage 2 mechanics preserved verbatim:
  - Windows via `_windows.rolling_windows()` (WIN_A_LO/HI, WIN_B_LO/HI,
    WIN_FULL_LO/HI). Rolls forward daily off `date.today()`.
  - L2-only: reads `forecast_l2`, never Production, never top-level
    `forecast`. Correction is `fc_l2 + mean_prior_14d(daily_res[(date-lag, hour)])`
    where `daily_res[(date, hour)] = mean(obs - fc_l2)`.
  - Per-cell verdict via `cell_verdict()` on (regime × lead_band): SHIP if
    halves-stable + full window ≥ 3% improve; MARGIN if full wins but one
    half is thin; SKIP if full loses or halves disagree in sign; THIN if
    n_full < 200.
  - Emits `analysis/output/h_{field}_residual_persistence_stage2.txt` plus
    the collector-facing `weather_collector/data/{field}_residual_persistence_curated.json`.

Refactor discipline: byte-for-byte equivalent output for dp and wg (proven
via before/after JSON diff at ship time). If a field constant needs
per-field variation (e.g. units_label), it becomes a `run_stage2` argument
rather than a fork of the harness.
"""
import json
import os
import sys
...
def _compute(field, win_a_lo, win_a_hi, win_b_lo, win_b_hi):
    path = cached_path(URL)

    # Pass 1: build daily_res[(date, hour)] = mean signed L2 residual
    print(f"[1/2] Building {field} daily L2-residual index...", file=sys.stderr)
    ...
    # Runtime hourly correction table
    ...
    # Pass 2: score baseline vs gated per (window, regime, lead_band)
    print(f"[2/2] Scoring {field} scenarios per (regime × lead_band)...", file=sys.stderr)
    ...
    print(f"    scored {n_rows:,} {field} rows; {n_no_prior:,} skipped (no prior residual)",
          file=sys.stderr)
...
def _cell_verdict(base_f, gate_f, base_a, gate_a, base_b, gate_b, n_full):
...
def _emit(field, units_label, accum, hourly_corr, hourly_corr_meta, fit_asof,
          win_a_lo, win_a_hi, win_b_lo, win_b_hi, win_full_lo, win_full_hi,
          out_txt, out_json):
    ...
    lines = []
    lines.append("=" * 100)
    lines.append(f"{field} RESIDUAL-PERSISTENCE — Stage 2 preview (per regime × lead_band)")
    lines.append("=" * 100)
    lines.append("")
    ...
    for regime in regimes:
        for band in bands:
            base_f = _mae(accum[("FULL", regime, band)], "ae_base")
            gate_f = _mae(accum[("FULL", regime, band)], "ae_gate")
            base_a = _mae(accum[("A", regime, band)], "ae_base")
            gate_a = _mae(accum[("A", regime, band)], "ae_gate")
            base_b = _mae(accum[("B", regime, band)], "ae_base")
            gate_b = _mae(accum[("B", regime, band)], "ae_gate")
            n_full = accum[("FULL", regime, band)]["n"]
            if n_full == 0 or base_f is None or gate_f is None:
                continue
            verdict, d_full, d_a, d_b = _cell_verdict(
    ...
    payload = {
        ...
        "hourly_correction": {
            ...
            "hour_of_day_n_days": {str(h): hourly_corr_meta.get(h, {}).get("n_days", 0)
                                    for h in range(24)},
            "units": f"{units_label} (add to forecast_l2 for {field} on SHIP cells)",
            "note": (
        ...
        "notes": (
            ...
            f"cells fall back to L2 alone. Wire via a new {field}_residual_persistence "
...
def run_stage2(field, units_label, out_txt=None, out_json=None):
    """Run Stage 2 preview for `field`. `units_label` is the correction unit
    stamped into the curated JSON's `hourly_correction.units` string
    ("°F" for dp/h, "mph" for wg). Returns 0."""
    if out_txt is None:
        out_txt = os.path.join(HERE, "output",
                               f"h_{field}_residual_persistence_stage2.txt")
    if out_json is None:
        out_json = os.path.abspath(os.path.join(
            HERE, "..", "weather_collector", "data",
            f"{field}_residual_persistence_curated.json"
        ))

    win_a_lo, win_a_hi, win_b_lo, win_b_hi, win_full_lo, win_full_hi = rolling_windows()

    accum, hourly_corr, hourly_corr_meta, fit_asof = _compute(
        field, win_a_lo, win_a_hi, win_b_lo, win_b_hi
    )
    _emit(field, units_label, accum, hourly_corr, hourly_corr_meta, fit_asof,
          win_a_lo, win_a_hi, win_b_lo, win_b_hi, win_full_lo, win_full_hi,
          out_txt, out_json)
    return 0
