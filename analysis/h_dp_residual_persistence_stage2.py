"""Stage 2 preview — dp residual-persistence per-cell verification.

Follow-on to Stage 1 (h_dp_residual_persistence_stage1.py), which found the
uniform-mean rolling residual over the prior 14d at the same clock hour,
applied on top of L2, reduces held-out dp MAE +11.14% pooled (4/6 regimes
WIN — losers were calm and se_flow; halves both positive at +3.81% and +4.74%).
Mirrors h_wg_residual_persistence_stage2.py exactly (same template, different
field). See project_todo.md item 7b.
...
Emits:
  analysis/output/h_dp_residual_persistence_stage2.txt
  weather_collector/data/dp_residual_persistence_curated.json   (preview, not wired)
"""
import os, sys, json
...
OUT_TXT = os.path.join(HERE, "output", "h_dp_residual_persistence_stage2.txt")
OUT_JSON = os.path.abspath(os.path.join(
    HERE, "..", "weather_collector", "data", "dp_residual_persistence_curated.json"
))
...
def compute():
    ...
    # from ALL dp rows across full available history. Correction skips lag=0, so
    # test-window rows use only prior days' residuals (no leak).
    print("[1/2] Building dp daily L2-residual index...", file=sys.stderr)
    ...
    print("[2/2] Scoring dp scenarios per (regime × lead_band)...", file=sys.stderr)
    ...
    print(f"    scored {n_rows:,} dp rows; {n_no_prior:,} skipped (no prior residual)",
          file=sys.stderr)
...
def emit(accum, hourly_corr, hourly_corr_meta, fit_asof):
    ...
    lines = []
    lines.append("=" * 100)
    lines.append("dp RESIDUAL-PERSISTENCE — Stage 2 preview (per regime × lead_band)")
    lines.append("=" * 100)
    lines.append("")
    ...
    payload = {
        ...
        "hourly_correction": {
            ...
            "hour_of_day_n_days": {str(h): hourly_corr_meta.get(h, {}).get("n_days", 0)
                                    for h in range(24)},
            "units": "°F (add to forecast_l2 for dp on SHIP cells)",
            "note": (
        ...
        "notes": (
            ...
            "cells fall back to L2 alone. Wire via a new dp_residual_persistence "
