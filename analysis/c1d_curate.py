"""
C1d confidence-layer — Stage 2 curated table.

Mirrors c1_curate_confidence_table.py. Reads c1d_confidence_premium.json
(Stage 1 output) and applies the standard curation rules to produce
weather_collector/data/c1d_curated.json for the runtime to consume.

Slot names: "low" / "high" (the C1d axis is cloud KBOS-vs-KBVY σ).

Run:
    python3 analysis/c1d_curate.py
"""
import json
import os
import sys
...
STAGE1_PATH = os.path.join(os.path.dirname(__file__), "output", "c1d_confidence_premium.json")
OUTPUT_PATH = os.path.join(
    os.path.dirname(os.path.dirname(os.path.abspath(__file__))),
    "weather_collector", "data", "c1d_curated.json",
)
...
def classify(cell):
    ...
    if n_low < SAMPLE_FLOOR or n_high < SAMPLE_FLOOR:
        return "SKIP", f"sample floor (n_low={n_low}, n_high={n_high})"
...
def curate():
    ...
    for field, bands in cells.items():
        ...
        for band, c in bands.items():
            ...
            sigma_q1 = c.get("sigma_q1", sigma_q1)
            sigma_q3 = c.get("sigma_q3", sigma_q3)
        find_outliers(curated[field])

    return {
        "generated_at": datetime.now(timezone.utc).isoformat(),
        "source": os.path.basename(STAGE1_PATH),
        "axis": "c1d_cloud_disagreement",
        "slots": ["low", "high"],
        "sigma_cuts": {"q1": sigma_q1, "q3": sigma_q3},
        "curation_rules": {
...
def print_summary(curated):
    field_order = ["cc", "cl", "cm", "ch"]
    field_order = [f for f in field_order if f in curated["cells"]]
    print("C1d confidence-layer Stage 2 curated table")
...
def main():
    if not os.path.exists(STAGE1_PATH):
        sys.exit(f"Missing Stage 1 input {STAGE1_PATH} — run c1d_calibration.py first")
    curated = curate()
