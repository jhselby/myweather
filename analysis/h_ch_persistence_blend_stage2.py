"""Stage 2 preview — ch regime-gated persistence blend.

Follow-on to Stage 0-1 (h_ch_persistence_blend.py), which verified on
halves-stability that a regime-gate (frontal→L4, else persistence) beats
baseline pooled ch MAE by ~20%.
...
second condition (frontal → L4, lead>=24 → L4, else persistence).

Windows match Stage 1 exactly (30d well-stamped, split into recent 15d
+ prior 15d halves) so the pooled numbers reconcile.

Persistence definition: flat carry of the joined ch observation at
...
Emits:
  analysis/output/h_ch_persistence_blend_stage2.txt
  weather_collector/data/ch_persistence_gate_curated.json   (preview, not wired)
"""
...
def compute():
    ...
    # accum[(window, scenario, regime, band)] = {n, ae, se}
    print("[2/2] Scoring ch scenarios per (regime × lead_band)...", file=sys.stderr)
    ...
    print(f"    joined {n_joined:,} ch rows; {n_orphan:,} orphans", file=sys.stderr)
    return accum
...
def cell_verdict(base_full, gate_full, base_a, gate_a, base_b, gate_b, n_full):
    ...
    # SHIP: full beats floor, both halves also beat (negative Δ means gate better)
...
def emit(accum):
    # Collect regimes seen
    regimes = sorted({key[2] for key in accum.keys()})
    bands = [name for name, _, _ in LEAD_BANDS]

    lines = []
    lines.append("=" * 100)
    lines.append("ch REGIME-GATED PERSISTENCE BLEND — Stage 2 preview (per regime × lead_band)")
    lines.append("=" * 100)
    lines.append("")
    lines.append("Gate under test: forecast = L4 if regime==frontal else persistence-of-obs.")
    lines.append("Per-cell verdict requires halves-stability + full window > 3.0% improvement.")
    lines.append("")

    # === Per-cell table: baseline vs regime_gate ===
    lines.append("=" * 100)
    ...
    # === Persistence-only comparison (for landmark-thread visibility) ===
    lines.append("=" * 100)
    ...
    header2 = (f"{'regime':<12}{'band':<8}{'n':>8}"
               f"{'base MAE':>10}{'pers MAE':>10}{'Δ full %':>10}  note")
    ...
    # Detect skip patterns for gate shape
    ...
    lines.append("=" * 100)
    # frontal → L4 (never persistence, by design)
    # skipped cells → fall back to L4
    # all other SHIP + MARGIN cells → persistence
    lines.append("  base rule: forecast = persistence-of-obs (flat carry)")
    lines.append("  fall back to L4 when:")
    lines.append("    - regime == 'frontal' (design; L4 is the only winner there)")
    if skip_cells:
        for regime, band in sorted(skip_cells):
            lines.append(f"    - regime == '{regime}' AND lead_band == '{band}'  (Stage 2 SKIP)")
    else:
        lines.append("    (no additional per-cell skips required — clean gate)")
    lines.append("")
    ...
    payload = {
        ...
        "gate": {
            "persistence_source": "hour-floor of run_time in ch obs index (matches Stage 1)",
            "frontal_uses_L4": True,
            "additional_skips": [
        ...
        "notes": (
            ...
            "of the joined ch obs at forecast issue time. Cells with verdict=SHIP or "
            "MARGIN use persistence at runtime; SKIP + frontal fall back to L4. "
            "Wire via a new ch_persistence_gate processor with ENABLED=False on ship "
            "and a 7-day gate before enabling."
        ),
    }
