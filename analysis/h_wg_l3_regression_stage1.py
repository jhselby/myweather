"""Stage 1 preview — wg L3 skip-table halves-verified per-cell verdict.

Follow-on to Stage 0 (h_wg_l3_regression.py), which flagged 10-11 cells
where wg L3 regresses (calm all bands +23-77%, unknown 3 bands +22-38%,
sea_breeze 6-11h + 24-47h, ne_flow 6-11h). Same skip-table architecture as
ws L3 (already shipped v0.6.279): drop L3 in specific (regime, lead_band)
cells where L3 hurts stably, keep it elsewhere.

Stage 1 answers: does the L3-hurts verdict hold on BOTH halves + full 30d
window per (regime × lead_band)? Halves-instability is the sw_flow
long-lead pattern from the ch persistence gate — Stage 0 pooled numbers
hide it; Stage 1 exposes it.

Windows mirror ch Stage 2 exactly (30d, split into recent 15d + prior 15d
halves) so the pipeline reads consistently.
...
Emits:
  analysis/output/h_wg_l3_regression_stage1.txt
  weather_collector/data/wg_l3_skip_table_curated.json   (preview, not wired)
"""
import os, sys, json
...
OUT_TXT = os.path.join(HERE, "output", "h_wg_l3_regression_stage1.txt")
OUT_JSON = os.path.abspath(os.path.join(
    HERE, "..", "weather_collector", "data", "wg_l3_skip_table_curated.json"
))
...
WIN_A_LO, WIN_A_HI = "2026-07-13T00:00", "2026-07-28T00:00"
WIN_B_LO, WIN_B_HI = "2026-06-28T00:00", "2026-07-13T00:00"
WIN_FULL_LO, WIN_FULL_HI = "2026-06-28T00:00", "2026-07-28T00:00"
...
def emit(accum):
    ...
    lines = []
    lines.append("=" * 100)
    lines.append("wg L3 SKIP-TABLE — Stage 1 preview (halves-verified per regime × lead_band)")
    lines.append("=" * 100)
    lines.append("")
    ...
    lines.append(f"PERSISTENCE_TERRITORY = persistence beats L2 in full window; belongs to wg residual persistence gate, not L3 skip table.")
    ...
    payload = {
        ...
        "notes": (
            ...
            "SKIP_TABLE[('wg','l3')] in decay_apply.py once the 7-day streak + "
            "no-halves-flip gate clears. PERSISTENCE_TERRITORY cells belong to "
            "the wg residual persistence gate discussion, not the L3 skip table."
        ),
    }
