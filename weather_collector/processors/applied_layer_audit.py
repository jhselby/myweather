"""Applied-layer consistency checker — shared audit logic.

Runs both as a CLI (via `analysis/applied_layer_audit.py`, exit code + stdout)
and as a Fitter preflight (via `run_audit()` returning structured results).

Catches the class of mismatch that hid the ws L3 skip-table dormancy for 4
days after v0.6.279: correction-layer config declares that a field gets a
correction, but nothing in the pipeline actually writes the state the
correction reads from.
...
Non-goals:
  ...
  - Corrections-file completeness — belongs elsewhere in the Fitter.
"""
import os
import re

from .decay_apply import (
...
def _grep_writer(pattern):
    """Return list of (relpath, line_no, line) matching pattern under COLLECTOR_DIR."""
    hits = []
            ...
            with open(path, "r", encoding="utf-8") as fh:
                for i, line in enumerate(fh, 1):
                    ...
                    if stripped.startswith("#"):
                        continue
                    if rx.search(line):
                        rel = os.path.relpath(path, os.path.dirname(COLLECTOR_DIR))
...
def run_audit():
    """Structured entry point for programmatic callers (Fitter preflight).

    Returns (ok: bool, passes: list[str], failures: list[str]).
    """
    a_pass, a_fail = check_config_coherence()
    b_pass, b_fail = check_state_read_writer()
    return (not (a_fail or b_fail)), (a_pass + b_pass), (a_fail + b_fail)


def format_report(passes, failures):
    """Return the human-readable report as a list of lines. Used by the CLI."""
    lines = []
    lines.append("=" * 72)
    lines.append("Applied-layer consistency audit")
    lines.append("=" * 72)

    a_pass, a_fail = check_config_coherence()
    lines.append("")
    lines.append("[A] Config coherence")
    lines.append("-" * 72)
    for line in a_pass:
    ...
    b_pass, b_fail = check_state_read_writer()
    lines.append("")
    lines.append("[B] State read/writer coherence")
    lines.append("-" * 72)
    for line in b_pass:
        lines.append(f"  ok    {line}")
    for line in b_fail:
    ...
    lines.append("=" * 72)
    if total_fail:
        lines.append(f"FAIL — {total_fail} problem(s)")
    else:
